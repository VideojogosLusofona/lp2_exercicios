...
namespace Exercise01
{
    class Program
    {
        private static void Main(string[] args)
        {
            if (args.Length < 2)
            {
                Console.WriteLine("Error! You must provide atleast 2 values.");
                return;
            }
        ...
        {
            1 => $"Average: {MathP.GetAverage(values)}",
            2 => $"Median: {MathP.GetMedian(values)}",
            3 => $"Mode: {MathP.GetMode(values)}",
            4 => $"Max: {MathP.GetMax(values)}",
            5 => $"Min: {MathP.GetMin(values)}",
...
namespace Exercise01
{
    public static class MathP
    {
        public static float GetAverage(int[] values) =>
            (float)GetSumTotal(values) / values.Length;
        ...
        public static float GetMedian(int[] values)
        {
            ...
            return values.Length % 2 != 0 ? values[middleIndex] :
                GetAverage(new int[2] { values[middleIndex], values[middleIndex - 1] });
        }

        public static int GetSumTotal(int[] values)
        {
            int total = 0;
        ...
        public static int GetMax(int[] values) => values.Max();

        // Uses Linq to determine min value
        public static int GetMin(int[] values) => values.Min();
    }
}
```
