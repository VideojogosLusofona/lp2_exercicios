## Problema

1 - Escreve um programa que aceite dois ou mais nÃºmeros inteiros como
parÃ¢metros na linha de comandos, coloque esses nÃºmeros num _array_ de `ints` e
apresente ao utilizador um menu com as seguintes opÃ§Ãµes:

1. Obter mÃ©dia dos valores no _array_
2. Obter mediana dos valores no _array_
3. Obter moda dos valores no _array_
4. Obter mÃ¡ximo dos valores no _array_
5. Obter mÃ­nimo dos valores _no_array_
6. Sair

O programa deve funcionar em ciclo, realizando os pedidos efetuados, terminando
apenas quando o utilizador selecionar a opÃ§Ã£o 6.

## Soluções

### Solução 1

Class Program:

```cs
using System;

namespace Ex1
{
    class Program
    {
        // Static para poder aceder a partir de outras classes
        public static int[] intigers;

        static void Main(string[] args)
        {
            // Criar uma nova instancia de Display
            Display display = new Display();

            // Colocar numeros num array de ints
            intigers = Array.ConvertAll(args, int.Parse);

            // Display the menu
            display.DisplayMenu();
        }
    }
}
```

Class Display:

```cs
using System;

namespace Ex1
{
    class Display
    {
        // Criar uma nova instancia de Calculo
        Calculo calculo = new Calculo();

        public Display() { }

        public void DisplayMenu()
        {
            Console.WriteLine("1 - Obter média dos valores no array");
            Console.WriteLine("2 - Obter mediana dos valores no array");
            Console.WriteLine("3 - Obter moda dos valores no array");
            Console.WriteLine("4 - Obter máximo dos valores no array");
            Console.WriteLine("5 - Obter mínimo dos valores no array");
            Console.WriteLine("6 - Sair");

            Choice(Convert.ToInt32(Console.ReadLine()));
        }

        private void Choice(int value)
        {
            switch(value)
            {
                case 1:
                    Console.WriteLine("Média: " + calculo.Media());
                    break;
                case 2:
                    Console.WriteLine("Mediana: " + calculo.Mediana());
                    break;
                case 3:
                    Console.WriteLine("Moda: " + calculo.Moda());
                    break;
                case 4:
                    Console.WriteLine("Máximo: " + calculo.Maximo());
                    break;
                case 5:
                    Console.WriteLine("Mínimo: " + calculo.Minimo());
                    break;
                case 6:
                    
                    break;
            }

            Console.ReadKey();
            DisplayMenu();
        }
    }
}
```

Class Calculo:

```cs
using System;
using System.Linq;

namespace Ex1
{
    class Calculo
    {
        public Calculo() { }


        public int Media()
        {
            int retVal = 0;

            // Somar todos os elementos do array
            foreach(int i in Program.intigers)
            {
                retVal += i;
            }

            // Dividir pelo total de elementos no array
            retVal /= Program.intigers.Length;

            return retVal;
        }

        public int Mediana()
        {
            int retVal = 0;

            // Organizamos on array com um Sort
            int[] ordIntigers = Program.intigers;
            Array.Sort(ordIntigers);

            // Guardamos o valor de centro do array numa variavel
            int mid = ordIntigers.Length / 2;

            // Calculamos a mediana dependendo se o tamanho do array é par ou impar
            retVal = (ordIntigers.Length % 2 != 0) ? 
                ordIntigers[mid] : (ordIntigers[mid] + ordIntigers[mid - 1]) / 2;

            return retVal;
        }

        public int Moda()
        {
            int retVal = 0;

            // Primeiro agrupamos todos os valores iguais
            retVal = Program.intigers.GroupBy(i => i)
            // Agora vamos buscar a frequencia com que aparece
            .OrderByDescending(n => n.Count())
            // Selecionamos a chave do grupo
            .Select(n => n.Key)
            // Por fim vamos buscar o valor mais frequente
            .FirstOrDefault();   

            return retVal;
        }

        public int Maximo()
        {
            int retVal = 0;

            // Ordenamos por ordem descendente e vamos buscar o primeiro valor
            retVal = Program.intigers.OrderByDescending(n => n).First();

            return retVal;
        }

        public int Minimo()
        {
            int retVal = 0;

            // Ordenamos por ordem ascendente e vamos buscar o primeiro valor
            retVal = Program.intigers.OrderBy(n => n).First();

            return retVal;
        }
    }
}
```

*Por [Leandro Brás](https://github.com/xShadoWalkeR)*