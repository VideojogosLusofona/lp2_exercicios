## Problema

19 - Considera a seguinte classe:

```cs
using UnityEngine;

public static class VectorOperations
{
    // Normalized direction between two game objects
    public static Vector2 Direction(Vector2 from, Vector2 to)
    {
        return (to - from) / (to - from).magnitude;
    }

    // Distance between two game objects
    public static float Distance(Vector2 obj1, Vector2 obj2)
    {
        return (obj1 - obj2).magnitude;
    }

    // Convert angle in degrees into normalized vector
    public static Vector2 Deg2Vec(float angle)
    {
        float angleRad = angle * Mathf.Deg2Rad;
        return new Vector2(Mathf.Cos(angleRad), Mathf.Sin(angleRad));
    }

    // Determine angle of vector in degrees
    public static float Vec2Deg(Vector2 vector)
    {
        return Mathf.Atan2(vector.y, vector.x) * Mathf.Rad2Deg;
    }
}
```

Responde às seguintes questões:

1. Simplifica os métodos usando notação lambda (`=>`).
2. Declara, para cada método, um _delegate_ personalizado compatível.
3. Indica, para cada método, um _delegate_ pré-definido do C# que seja
   compatível.
4. Assumindo que estás num método noutra classe, escreve quatro linhas de
   código nas quais declaras quatro variáveis do tipo _delegate_ pré-definido
   que indicaste na alínea anterior, atribuindo-lhes o respetivo método
   compatível.
5. Repete a alínea anterior mas considerando os _delegates_ personalizados que
   declaraste na alínea 2 (e não os _delegates_ pré-definidos do C#).

## Soluções

### Solução 1

#### Questões

1. Métodos Simplificados

```csharp
using UnityEngine;

public static class VectorOperations
{
    // Normalized direction between two game objects
    public static Vector2 Direction(Vector2 from, Vector2 to) => (to - from) / (to - from).magnitude;

    // Distance between two game objects
    public static float Distance(Vector2 obj1, Vector2 obj2) => (obj1 - obj2).magnitude;

    // Convert angle in degrees into normalized vector
    public static Vector2 Deg2Vec(float angle) => 
        new Vector2(Mathf.Cos(angle * Mathf.Deg2Rad), Mathf.Sin(angle * Mathf.Deg2Rad));

    // Determine angle of vector in degrees
    public static float Vec2Deg(Vector2 vector) => Mathf.Atan2(vector.y, vector.x) * Mathf.Rad2Deg;
}
```

2. *Delegates* acima de cada método

```csharp
using UnityEngine;

public static class VectorOperations
{
    public delegate Vector2 VectorDir(Vector2 from, Vector2 to);
    // Normalized direction between two game objects
    public static Vector2 Direction(Vector2 from, Vector2 to) => (to - from) / (to - from).magnitude;


    public delegate float VectorDistance(Vecotr2 obj1, Vector2 obj2);
    // Distance between two game objects
    public static float Distance(Vector2 obj1, Vector2 obj2) => (obj1 - obj2).magnitude;


    public delegate Vector2 AngleToVector(float angle);
    // Convert angle in degrees into normalized vector
    public static Vector2 Deg2Vec(float angle) => 
        new Vector2(Mathf.Cos(angle * Mathf.Deg2Rad), Mathf.Sin(angle * Mathf.Deg2Rad));


    public delegate float VectorToAngle(Vector2 vector);
    // Determine angle of vector in degrees
    public static float Vec2Deg(Vector2 vector) => Mathf.Atan2(vector.y, vector.x) * Mathf.Rad2Deg;
}
```

3. *Delegates* pré-definidos por C#

```csharp
using UnityEngine;
using System;

public static class VectorOperations
{
    public Func<Vector2, Vector2, Vector2> VectorDir;
    // Normalized direction between two game objects
    public static Vector2 Direction(Vector2 from, Vector2 to) => (to - from) / (to - from).magnitude;


    public Func<Vector2, Vector2, float> VectorDisance;
    // Distance between two game objects
    public static float Distance(Vector2 obj1, Vector2 obj2) => (obj1 - obj2).magnitude;


    public Func<float, Vector2> AngleToVector;
    // Convert angle in degrees into normalized vector
    public static Vector2 Deg2Vec(float angle) => 
        new Vector2(Mathf.Cos(angle * Mathf.Deg2Rad), Mathf.Sin(angle * Mathf.Deg2Rad));


    public Func<Vector2, float> VectorToAngle;
    // Determine angle of vector in degrees
    public static float Vec2Deg(Vector2 vector) => Mathf.Atan2(vector.y, vector.x) * Mathf.Rad2Deg;
}
```

4. Uso dos *delegates* da alínea anterior

```csharp
Func<Vector2, Vector2, Vector2> direction = (f, t) => (t - f) / (t - f).magnitude;
Func<Vector2, Vector2, float> distance = (obj1, obj2) => (obj1 - obj2).magnitude;
Func<float, Vector2> angleVector = a => new Vector2(Mathf.Cos(a * Mathf.Deg2Rad), Mathf.Sin(a * Mathf.Deg2Rad));
Func<Vector2, float> vectorAngle = v => Mathf.Atan2(v.y, v.x) * Mathf.Rad2Deg;
```

5. Uso dos *delegates* definidos na alínea 2

```csharp
VectorDir direction = (f, t) => (t - f) / (t - f).magnitude;
VectorDistance distance = (obj1, obj2) => (obj1 - obj2).magnitude;
AngleToVector angleVector = a => new Vector2(Mathf.Cos(a * Mathf.Deg2Rad), Mathf.Sin(a * Mathf.Deg2Rad));
VectorToAngle vectorAngle = v => Mathf.Atan2(v.y, v.x) * Mathf.Rad2Deg;
```