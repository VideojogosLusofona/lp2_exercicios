...
* Tendo em conta de isto, pode haver casos em que queremos uma classe que
têm **nada** em comum com `Human` e `Orc`, com as suas próprias caraterísticas.

* Por exemplo, temos uma classe chamada `DifferentBeast` que têm `NPC` como
herança e só têm um `int` chamado `Weirdness`. Vamos criar uma instância desta
classe em `Program` e adicionar-lâ a nossa lista de `NPC`. Ao correr o método
`ToString()` desta classe, o *output* será os três campos de `NPC` igualados
a 0. Como se pode ver, estes três campos estão sempre presentes ao fazer
herança, que não queremos. Por isso devemos programar para composições e não
heranças.

* Em conclusão, esta solução funciona se já sabemos que todas as classes
derivadas de `NPC` vão ter algo em comum (todas têm que ter `Strength`,
`Dexterity` e `Charisma`), com as suas diferenças adicionais sendo próprias
aquela classe. Se não for o caso, teremos que encontrar outra solução que
encaixe bem ao o que nós queremos.
